...
import { DEFAULT_LABORATORY_PROFILE, loadLaboratoryProfile, saveLaboratoryProfile, type LaboratoryProfile } from "./branding";
...
export function BrandingProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo(() => ({ profile, hydrated: true, updateProfile(next: LaboratoryProfile) { saveLaboratoryProfile(next); setProfile(next); }, restoreDefault() { const next = { ...DEFAULT_LABORATORY_PROFILE }; saveLaboratoryProfile(next); setProfile(next); } }), [profile]);
