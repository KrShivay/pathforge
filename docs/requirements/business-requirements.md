...
**This is reference material, not the project entry point.** Read the [project brief](../README.md) first, then [Architecture invariants](architecture-invariants.md). Consult this document for detail on specific topics; you are not expected to retain it uniformly.
...
1. **Section 16 (Data Integrity Invariants) is removed** and replaced by the pointer block below. The authoritative, numbered invariants live only in [Architecture invariants](architecture-invariants.md) (INV-1 … INV-10). Do not restate or renumber them here — a second copy will drift. Refer to them everywhere as `INV-n`.
...
- Resolved values & historical meaning → §8 (the *why identifier-alone is insufficient* argument; see [Catalog V1 → V2 change notes](../fixtures/catalog-change-notes.md) for the chosen snapshot mechanism)
...
- **Invariants → [Architecture invariants](architecture-invariants.md)** (was §16)
...
- Agent deliverables & context prompt → §36–§38 (the §37 prompt is condensed into the [project brief](../README.md))
